...
import { client } from "./gls";

export default class GizCompilerProvider implements vscode.CodeActionProvider {
  ...
  public activate(subscriptions: vscode.Disposable[]) {
    ...
    this.buildDiagnostics = vscode.languages.createDiagnosticCollection("giz");
    this.astDiagnostics = vscode.languages.createDiagnosticCollection("giz");
  ...
  maybeDoASTGenErrorCheck(change: vscode.TextDocumentChangeEvent) {
    if (change.document.languageId !== "giz") return;
    if (vscode.workspace.getConfiguration("giz").get<string>("astCheckProvider", "gls") !== "extension") {
  ...
  private _doASTGenErrorCheck(change: vscode.TextDocumentChangeEvent) {
    ...
    if (textDocument.languageId !== "giz") {
      return;
    }
    const giz_path = config.get("gizPath") || "giz";
    const cwd = vscode.workspace.getWorkspaceFolder(textDocument.uri).uri
      .fsPath;

    let childProcess = cp.spawn(giz_path as string, ["ast-check"], { cwd });
  ...
  private _doCompile(textDocument: vscode.TextDocument) {
    ...
    let decoded = "";
    let childProcess = cp.spawn("giz", processArg, { cwd });
    if (childProcess.pid) {
